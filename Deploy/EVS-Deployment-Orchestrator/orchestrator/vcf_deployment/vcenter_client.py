...
# The local VMFS we remove sits on a 256 GB EBS volume (Phase 2 default -- see
# evs_environment/ebs_volume_manager.py ``_DEFAULT_VOLUME_SIZE_GB``). VMFS
# reports usable capacity slightly under the raw size (metadata overhead; 256
# GB surfaces as ~255 GB), so match within a few-GB tolerance.
...
class VcenterClient:
    ...
    Args:
        ...
        verify_tls: False to disable TLS verification entirely (vCenter
            ships with a self-signed cert unless separately configured).
            True to use the system default CA trust store (fails against a
            self-signed cert - only useful if a real CA-issued cert is
            configured). Or a filesystem path (str) to a PEM cert to trust
            specifically - this is the recommended mode for a self-signed
            appliance: real certificate validation against the exact
            pinned cert, not a blanket "skip validation" bypass.
    ...
    def connect(self) -> None:
        ...
        if isinstance(self._verify_tls, str):
            # Pinned-cert mode: trust exactly this one cert file -- real chain
            # validation against the pinned cert, not a blanket "skip
            # validation" bypass like CERT_NONE.
            context = ssl.create_default_context(cafile=self._verify_tls)
        elif self._verify_tls:
            context = ssl.create_default_context()
        else:
            context = ssl._create_unverified_context()

        logger.info("Connecting to vCenter at %s", self._host)
        # httpConnectionTimeout defaults to None (unbounded) in pyvmomi -- a
        # hung vCenter would block forever with no other safety net. Bound it
        # explicitly; 60s is generous for a same-VPC connection.
    ...
    def find_vsan_datastore_on_cluster(self, cluster_name: str) -> Any:
        ...
        Edge VMs MUST deploy on vSAN -- the per-host local VMFS is cluster-local
        (won't survive vMotion/host failure) and is only a transient landing
        spot for the installer OVA; anything else risks stranding edges when the
        EBS-backed datastore is cleaned up post-bringup. First vSAN match wins.
    ...
    def remove_local_installer_datastore(self, cluster_name: str) -> dict[str, Any]:
        ...
        Post-bringup the mgmt cluster has the cluster-wide vSAN datastore plus
        a local VMFS on a 256 GB EBS volume -- a one-time landing spot for the
        installer OVA, now dead weight. We identify it by capacity match
        (non-vSAN AND within ±5 GB of ``_INSTALLER_DATASTORE_CAPACITY_GB``;
        VMFS reports slightly under the raw EBS size); name-based matching is
        more brittle since the operator names the datastore by hand at bringup.
        ...
        Doesn't touch AWS: the EBS volume stays attached. Run Phase 3's
        ``destroy-ebs-volume`` (``ebs_volume_destroyer.py``) afterward to
        detach + delete it.
        ...
        # 1. Collect cluster datastores matching the installer-VMFS profile
        #    (non-vSAN AND capacity within tolerance of the 256 GB EBS size).
        #    Dedupe by managed-object id (a shared datastore appears on
        #    multiple hosts; we refuse those below anyway).
        target_lo = (
        ...
        # 3. Storage-vMotion any VMs off the target onto vSAN. Resolve the
        #    vSAN datastore up-front so we fail fast if it isn't there
        #    (find_vsan_datastore_on_cluster raises when absent).
    ...
    def _storage_vmotion(self, vm: Any, target_datastore: Any) -> None:
        ...
        Synchronous; blocks until the RelocateVM_Task terminates and raises
        ``RuntimeError`` on failure (bubbles up so partial evacuations fail
        cleanly).
        """
    ...
    def _wait_for_task(self, task: Any, description: str,
                        timeout_seconds: int = 1800) -> Any:
        """Block until a vSphere task reaches success or error.

        Bounded by ``timeout_seconds`` (default 30 min): a stuck task (storage-
        vMotion, DVS reconfig, an unresolved DRS rule) would otherwise block the
        orchestrator forever with no safety net.
        """
        import time
        ...
        while task.info.state not in (
            ...
            if time.time() >= deadline:
                raise TimeoutError(
                    f"vCenter task did not complete within {timeout_seconds}s "
                    f"({description}); current state: {task.info.state}"
                )
            time.sleep(2)

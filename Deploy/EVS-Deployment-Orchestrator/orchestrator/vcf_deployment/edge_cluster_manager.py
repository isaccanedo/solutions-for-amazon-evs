...
class EdgeClusterManager:
    ...
    def load_spec(self) -> dict[str, Any]:
        ...
        # Resolve ``__SECRET:<role>__`` placeholders against Secrets Manager so
        # the in-memory spec carries real passwords while the on-disk JSON stays
        # inert. Phase 2 emits the edge password as a placeholder; the env id at
        # the top level tells the resolver where to look.
        ...
        # Fail closed: a literal __SECRET:<role>__ must never reach NSX/edge as
        # a password. If any placeholder survived (no SM client, unknown env, or
        # an unresolvable/tuple-nested value), stop here rather than configure
        # edge nodes with the literal placeholder string.
        if "__SECRET:" in json.dumps(spec):
            raise RuntimeError(
                "Edge cluster spec still contains __SECRET: placeholders after "
                "resolution — refusing to deploy with unresolved secrets "
                f"(environmentId={env_id!r}, "
                f"secrets_manager_wired={self._sm_client is not None})."
            )
    ...
    def _create_edge_tep_ip_pool(self, spec: dict[str, Any]) -> None:
        ...
        # carries the CIDR/gateway/range; its resource_type drives the SDK's
        # polymorphic type discriminator (parent type IpSubnets).
    ...
    def _create_uplink_profile(self, spec: dict[str, Any]) -> None:
        ...
        FAILOVER_ORDER with a single active uplink (no standby), matching our
        one-uplink-per-edge design.

        Idempotent via GET-first: NSX 9.x rejects PATCH on an existing
        host-switch-profile with ``error_code 500127`` ("... already exists"),
        unlike the IP pool + TZ resources which upsert fine, so short-circuit
        when the profile is already there. The GET guard should treat only a
        NotFound as "absent"; swallowing AccessDenied/throttling/timeout would
        fall through to the PATCH NSX rejects, defeating idempotency on
        --resume.
        """
        ...
        try:
            ...
            # NOTE: the vcf.nsx SDK raises vcf.nsx.model_client.ApiError on a
            # not-found GET with an EMPTY body (every field None), which is
            # indistinguishable from any other ApiError — so we CANNOT narrow to
            # a not-found type here. (Narrowing to vAPI NotFound was tried and
            # let the empty ApiError propagate, failing the edge stage after a
            # successful bringup.) Treat any GET failure as "absent" and let the
            # subsequent create/upsert surface a genuine error if one exists.
            existing = None
    ...
    def _create_vlan_transport_zone(self, spec: dict[str, Any]) -> None:
        ...
        try:
            ...
            # See _create_uplink_profile: vcf.nsx raises an empty ApiError on a
            # not-found GET, so narrowing isn't possible — treat any failure as
            # "absent" and let the create/upsert below surface real errors.
            existing = None
        if existing is not None:
            logger.info(
    ...
    def deploy_edge_nodes(self, dry_run: bool = False) -> None:
        ...
        # Cross-cutting NSX resource paths (Stage 1) and vCenter MOIDs every
        # edge body needs. Stage 2 uses the management API, which references
        # resources by UUID, not policy path (Stages 4-7 use the policy API and
        # its ``find_*_path`` helpers). A policy path to the mgmt API surfaces
        # as ``error_code 5008 "Pool identifier is null."`` from id-allocation.
        ...
        # SSH to the edges is off by default (opt-in via enableSsh), mirroring
        # jumpbox.enabled — edges sit on the mgmt VLAN, where leaving root SSH
        # open would invite brute-force.
        enable_ssh = bool(spec.get("enableSsh", False))
    ...
    def _deploy_single_edge(
        ...
        if existing is not None:
            existing_id = existing.node_id or existing.id
            try:
                # A name match alone isn't enough: a prior run could have POSTed
                # then failed realization (state "failed", timeout, transient
                # poll errors). Skipping on name alone would leave the failed
                # node (and its half-deployed edge VM) in place, and Stage 3
                # would group it in, surfacing later as a confusing BGP/edge-
                # policy timeout. So verify realization before skipping.
                self._nsx.wait_for_transport_node_state(existing_id)
                logger.info(
                    "Transport node '%s' already exists and is realized; "
                    "skipping create", fqdn,
                )
                return
            except Exception as e:  # noqa: BLE001
                logger.warning(
                    "Transport node '%s' already exists but did not reach "
                    "a successful realization state (%s) — deleting it "
                    "(and its vCenter edge VM) so it can be recreated "
                    "cleanly instead of leaving an orphaned failed node "
                    "in place.",
                    fqdn, e,
                )
                self._nsx.delete_transport_node(existing_id, force=True)
        ...
        edge_node = nsx_m.EdgeNode(
            ...
            deployment_config=nsx_m.EdgeNodeDeploymentConfig(
                ...
                # vm_deployment_config is typed as ``DeploymentConfig``; build
                # the concrete ``VsphereDeploymentConfig`` and upcast so the SDK
                # converter accepts it (see the convert_to note below).
            ...
            node_settings=nsx_m.EdgeNodeSettings(
                hostname=fqdn,
                # Off by default (opt-in via enable_ssh): the admin CLI user
                # suffices for normal operation, and root SSH is an unnecessary
                # attack surface on the management VLAN, matching NSX hardening
                # guidance.
                allow_ssh_root_login=enable_ssh,
                enable_ssh=enable_ssh,
        ...
        # TransportNode has three fields typed as polymorphic abstract bases:
        #   - host_switch_spec    : HostSwitchSpec   (concrete: StandardHostSwitchSpec)
        #   - node_deployment_info: Node             (concrete: EdgeNode)
        #   - host switch ip_assignment_spec: IpAssignmentSpec (StaticIpPoolSpec)
        # The SDK's REST converter uses class equality (not isinstance), so a
        # concrete subclass raises ``CoreException: Expected class ... <base> but
        # received ... <concrete>``. ``convert_to(BaseType)`` upcasts while
        # keeping every field (incl. the ``resource_type`` discriminator), so
        # the wire JSON is byte-identical to what NSX expects.
    ...
    def create_edge_cluster(self, dry_run: bool = False) -> None:
        ...
        for node in spec["edgeNodes"]:
            # wait_for_ variant blocks until the mgmt-API listing shows the
            # transport node -- covers the chained ``deploy-edge-cluster`` run
            # where Stage 3 starts right after Stage 2's last POST and the
            # listing may lag. Single-stage runs return immediately.
    ...
    def _create_tier0_uplink_interfaces(
        ...
        Each interface must be pinned to its edge via ``edge_path``; otherwise
        both default to the same edge node and NSX 9.x rejects the second with
        ``error_code 503101 "Segment ... is already attached"`` (a segment
        attaches to only one interface per edge).
        """
    ...
    def _create_rfc1918_prefix_list(
        ...
        # Each entry is a dict with ``network`` plus optional ``ge``/``le``.
        # For backwards compat (hand-edited specs) a bare string is still
        # accepted, behaving as ``ge``/``le`` unset -- matches only the literal
        # supernet (see Phase 2 ``_RFC1918_PREFIXES`` for why ``ge`` matters).
        prefixes: list[Any] = []
        ...
        # Catch-all deny so any non-listed network isn't advertised. NSX 9
        # requires every PrefixEntry (even DENY) to carry a ``network`` field
        # (validation error 255 "required property network is missing"), so the
        # catch-all is explicit as 0.0.0.0/0.
    ...
    def _create_bgp_neighbors(
        ...
        Each edge peers to its Route Server endpoint IP from its own uplink IP.
        The outbound filter is the RFC-1918 prefix list directly (no route-map
        wrapper): NSX 9 accepts a prefix-list path in ``out_route_filters``.
        """
    ...
    def _configure_route_redistribution(
        ...
        ``LocaleServices`` (no separate endpoint); we PATCH the locale service,
        and the Stage 4 edge-cluster attachment survives because PATCH merges.

        Spec shape: a single ``sources`` list under ``tier0.routeRedistribution``
        (flattened across T0/T1 enums), emitted as one
        ``Tier0RouteRedistributionRule``. Backwards-compat: the old two-list
        shape (``tier0Sources``/``tier1Sources``) is merged, and the legacy
        3-boolean shape is translated; both still emit a single rule.
        """

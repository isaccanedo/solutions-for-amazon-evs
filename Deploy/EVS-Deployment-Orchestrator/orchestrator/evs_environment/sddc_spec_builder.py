...
Constructs ``vmware.vcf_installer.model_client.SddcSpec`` (and nested classes)
from the Phase 2 config dict. The result can be passed to
``Sddcs.deploy_sddc(sddc_spec)`` or serialized to wire JSON and written to
disk for inspection (what ``phase3_sync.py`` does).

Version handling: two fields aren't modeled as typed fields, so they're set
via ``VapiStruct._set_extra_fields({...})`` (added to the wire JSON verbatim;
see ``_version_extras``):

  - ``deployWithoutLicenseKeys`` — required on both 9.0 and 9.1.
...
class SddcSpecBuilder:
    ...
    # i4i.metal is deliberately NOT here: the installer infers Ice Lake, and
    # setting an EVC mode breaks VCF 9.1 (ESXi 9.1 VMs exceed the intel-icelake
    # baseline, so the late "Enable Cluster EVC Mode" fails
    # EVCAdmissionFailedVmActive). i7i needs it: bringup can't infer a Sapphire
    ...
    def _build_network_specs(self) -> list[m.SddcNetworkSpec]:
        ...
        core_pools = ("vmManagement", "vmkManagement", "vMotion", "vSan")
        missing_core = [key for key in core_pools if key not in vlans]
        if missing_core:
            missing = ", ".join(f"initialVlans.{key}" for key in missing_core)
            raise ValueError(
                f"{missing} missing from config — ensure config.json is "
                f"fully populated (initialVlans is required)."
            )
    ...
    def _build_host_specs(self) -> list[m.SddcHostSpec]:
        ...
        # ESXi root passwords use the same placeholder path as the appliance
        # passwords: Phase 3's secret_resolver swaps each ``__SECRET:esxi:
        # <host>__`` for the value in ``evs!<env>_<host>`` (created by EVS at
        # provision time), keeping real passwords out of bringup_spec.json.
        ...
        for short in esxi_hostnames:
            hosts.append(
                m.SddcHostSpec(
                    ...
                    # validator even though skipEsxThumbprintValidation is set.
                    # Null here (ESXi hosts aren't DNS-resolvable from Phase 2's
                    # workstation); Phase 3's BringupManager fills it in at POST.
    ...
    def _build_cluster_spec(self) -> m.SddcClusterSpec:
        # EVC mode must match the physical CPU generation. Without this,
        # bringup on i7i.metal-24xl fails within 10 minutes because VCF
        # either infers the wrong mode or leaves it unset.
        instance_type = self._config.get("evsInstanceType", "")
        evc_mode = self._EVC_MODE_BY_INSTANCE_TYPE.get(instance_type)

        spec = m.SddcClusterSpec(
        ...
        if evc_mode:
            spec.cluster_evc_mode = evc_mode
        return spec
    ...
    def _build_sddc_manager_spec(self) -> m.SddcManagerSpec:
        ...
        return m.SddcManagerSpec(
            ...
            # The installer reads VMwareProductVersion off the SDDC Manager
            # spec, not the top-level SddcSpec.version. Missing it is a hard 500
            # before validation: "VMwareProductVersion can not be null or empty".
    ...
    def _version_extras(self) -> dict[str, Any]:
        ...
        - ``deployWithoutLicenseKeys`` — required on the wire for both 9.0 and
          9.1, not modeled by the 9.1 SDK.
        - ``vcfOperationsFleetManagementSpec`` — required on 9.0, dropped on
          9.1; not modeled by the 9.1 SDK.
        - ``vspClusterSpec``, ``licenseServerSpec``, ``vidbSpec`` — required on
          9.1 (VCF Services Platform, License Server, Identity Broker).
        - ``__env__`` / ``__region__`` — project-private extras (not VCF API)
          Phase 3 reads to fetch passwords from the right Secrets Manager
          namespace and region. The underscores avoid colliding with real
          installer fields and signal "internal use".
        """
        extras: dict[str, Any] = {
    ...
    def _build_vsp_cluster_spec(self) -> "m.SddcVspClusterSpec":
        ...
        hostnames = self._config.get("vcfHostnames", {})
        vlans = self._config.get("initialVlans", {})
        vm_mgmt_cidr = vlans.get("vmManagement", {}).get("cidr")
        if not vm_mgmt_cidr:
            raise ValueError(
                "initialVlans.vmManagement.cidr missing from config — "
                "ensure config.json is fully populated."
            )

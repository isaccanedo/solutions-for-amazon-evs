...
from src.bringup_manager import BringupManager
from src.depot_manager import DepotManager
from src.ebs_volume_destroyer import EbsVolumeDestroyer
from src.edge_cluster_manager import EdgeClusterManager
from src.installer_client import InstallerClient
from src.nsx_client import NsxClient
from src.secret_resolver import MissingSecretsError, SecretResolver, secret_name
...
# Default spec paths. Resolved relative to this source file's location
# (repo root → Phase_3_VCF9/<name>.json). Operators can override with
# the BRINGUP_SPEC_PATH / EDGE_CLUSTER_SPEC_PATH env vars if they want
# the specs read from somewhere else.
_DEFAULT_BRINGUP_SPEC_PATH = (
    os.environ.get("BRINGUP_SPEC_PATH")
    or str(Path(__file__).resolve().parents[2] / "bringup_spec.json")
)
_DEFAULT_EDGE_CLUSTER_SPEC_PATH = (
    os.environ.get("EDGE_CLUSTER_SPEC_PATH")
    or str(Path(__file__).resolve().parents[2] / "edge_cluster_spec.json")
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--installer-password", default=None)

    # Depot / bundle / release flags (depot actions)
    parser.add_argument(
        "--depot-token",
        default=None,
        help="Broadcom download token for configure-depot. "
             "Falls back to the VCF_DEPOT_TOKEN env var.",
    )
    parser.add_argument(
        "--depot-username",
        default=None,
    ...
    parser.add_argument(
        "--depot-password",
        default=None,
        help="Optional Broadcom account password for configure-depot.",
    )
    parser.add_argument(
        "--wait",
        action="store_true",
...
def _read_spec_json(spec_path: str) -> dict:
    ...
    Used by the one-click SDDC + NSX deployment precheck to gather
...
def _env_id_from_specs(args: argparse.Namespace) -> str | None:
    ...
    Bringup spec carries it as ``__env__``; edge cluster spec carries it
    as ``environmentId``. Either is fine — both are written by the same
    Phase 2 ``post-evs-sync-config`` run. Returns ``None`` if neither
    spec is present or populated, in which case Secrets Manager fallback
    is skipped and we fall through to the prompt.
    """
...
def _region_from_specs(args: argparse.Namespace) -> str | None:
    ...
    Bringup spec carries it as ``__region__``; edge cluster spec carries
    it as ``region``. Both are written by Phase 2 from
    ``config.json:region``. Returns ``None`` if neither spec has it
    populated, in which case ``_build_secrets_manager_client`` falls
    back to ``--aws-region`` / env vars / the boto3 default chain.
    """
    bringup_path = args.spec_path or _DEFAULT_BRINGUP_SPEC_PATH
    edge_path = _DEFAULT_EDGE_CLUSTER_SPEC_PATH
    bringup_dict = _read_spec_json(bringup_path)
...
def _cluster_name_from_bringup_spec(args: argparse.Namespace) -> str | None:
    ...
    Lets ``remove-installer-datastore`` and any other cluster-scoped
    action skip the ``--cluster-name`` flag in the common case where
    the operator is targeting the management cluster Phase 2
    provisioned. Returns ``None`` if the spec isn't present or the
    field is missing — caller falls back to the explicit flag.
    """
    bringup_path = args.spec_path or _DEFAULT_BRINGUP_SPEC_PATH
    bringup_dict = _read_spec_json(bringup_path)
...
def _build_secrets_manager_client(args: argparse.Namespace):
    ...
    Returns ``None`` when ``--no-secrets-manager`` is set so callers can
    short-circuit. Region resolution order: ``--aws-region`` flag,
    ``$AWS_REGION``, ``$AWS_DEFAULT_REGION``, the spec files
    (``__region__`` on bringup, ``region`` on edge cluster, both written
    by Phase 2 from ``config.json:region``), and finally ``us-east-1``.
    Spec-derived region is the path that's actually exercised in
    practice — operators rarely need to set ``--aws-region`` because the
    EVS environment's region is already baked into the specs.

    The default credential chain (env vars, instance profile,
    ``~/.aws/credentials``) handles authentication; we just provide
    profile + region override hooks.
    """
...
def _resolve_password_with_sm(
    ...
    Used by NSX Manager + vCenter handlers, where the password being
    looked up is the same one bringup deployed (``nsxAdmin`` /
    ``vcenterSso``). When SM is wired up and the env id is known,
    skip the prompt and fetch directly. Falls back to the prompt only
    if SM lookup itself fails (network error, missing secret).

    When ``cache_to_sm`` is True and the value comes from CLI / env /
    prompt (i.e. SM didn't have it), the resolved value is written to
    Secrets Manager under ``evs-<env>_<role>``. Used for the
    VCF Installer ``admin@local`` password — this isn't auto-generated
    by Phase 2 (the operator types it during OVA deploy), but once
    typed once, we can store it and skip the prompt forever after.
    Creates the secret if it doesn't exist; updates the value if it
    does.
    """
...
def _stash_secret(sm_client, env_id: str | None, role: str, value: str) -> None:
    ...
    Creates the secret if it doesn't exist; updates the value if it
    does. Soft-fails — we don't want a stash failure to break the
    bringup that just authenticated successfully.
    """
    from botocore.exceptions import ClientError
...
def _handle_bringup(args: argparse.Namespace) -> int:
    ...
    if args.action == "start-bringup":
        ...
        # Surface a non-zero exit when the workflow actually ended in
        # failure. Without this, the one-click SDDC + NSX deployment
        # would treat a failed bringup as "step 2 succeeded" and march
        # into step 3.
        # ``manager.start()`` only returns a status when ``--wait`` is
        # set; a no-wait start returns immediately with whatever the
        # initial POST response said, so we don't second-guess the
        # caller in that mode.
        if args.wait:
...
def _handle_prepare_depot(
    ...
    Same arguments as the individual actions: takes ``--depot-token`` /
    ``$VCF_DEPOT_TOKEN`` for step 1 and ``--target-version`` for step 3.
    Both ``--wait`` substeps poll internally — ``--wait`` is implicit on
    this action because partial pre-bringup state isn't useful.

    For operators who want fine-grained control (e.g. inspect the catalog
    between sync and download), the three individual actions are still
    available.
    """
...
def _handle_download_all_product_binaries(
    ...
    The installer's catalog puts actual product identity on
    ``components[0].type`` (VROPS, VCENTER, NSX_T_MANAGER, SDDC_MANAGER,
    VCF_OPS_CLOUD_PROXY, VRSLCM, VRA, HOST, NSX_ALB, ...). The top-level
    ``bundle.type`` is almost always the useless umbrella value
    ``VMWARE_SOFTWARE``, so we group by component type instead.
    ...
    #    components at all (i.e. PATCH/UPGRADE-only bundles), and skip
    #    patch-level INSTALL images (e.g. 9.0.2.0200) which the installer
    #    rejects for fresh deployments.
    ...
    # 3. Poll until every bundle lands in a terminal state.
...
def _handle_aws(args: argparse.Namespace) -> int:
    ...
    Currently just ``destroy-ebs-volume`` — detach + delete the EBS
    volume Phase 2 attached for the VCF Installer VMFS. Identifies the
    volume by tag (``ManagedBy=phase2-automation`` +
    ``EnvironmentId=<env>``) and refuses to act on anything else.

    The one-click SDDC + NSX deployment chains this in step 4 between
    ``remove-installer-datastore`` and ``deploy-edge-cluster``. Also
    callable standalone for cleanup-only runs.
    """
...
def create_ops_manager_connector(args: argparse.Namespace) -> int:
    ...
    logger.info("Polling connector %s until ACTIVE (timeout: 10 min)...", connector_id)
    ...
    logger.error("Connector %s did not reach ACTIVE within 10 minutes", connector_id)
    return 1
...
def _handle_pipeline(args: argparse.Namespace) -> int:
    ...
    Sequences the six Phase 3 stages end-to-end so the operator runs a
    single command from a populated config to a fully deployed VCF +
    edge cluster. Each substep delegates to its existing handler so
    logs and behavior match running the steps individually.
    ...
    Pipeline:
      ...
      5. ``deploy-edge-cluster`` — run all 7 NSX edge stages in order.
         Cross-stage NSX lookups use the poll-with-timeout helpers added
         under TODO so the chain doesn't race NSX's policy-side
    ...
    Step 2 always uses ``--wait`` regardless of the user-passed
    ...
    # Step 0: precheck — validate every required Secrets Manager secret
    # is present BEFORE we burn 30 minutes on prepare-depot. Reads the
    # bringup + edge specs as raw dicts, finds every ``__SECRET:<role>__``
    # placeholder, and calls ``DescribeSecret`` on each. Fails fast with
    # a single error listing every missing role. Doesn't fetch values
    # yet — plaintext stays out of memory until the per-stage handlers
    # need it.
    ...
    # Resolve the VCF Installer password once, after the precheck, before
    # the first handler grabs it. Both prepare-depot (step 1) and
    # start-bringup (step 2) auth to the installer and would otherwise
    # each prompt independently. Push the value into ``args`` so the
    # downstream handlers see it via the normal CLI-flag code path.
    #
    # Reuses the SM-aware resolver with ``cache_to_sm=True`` so the
    # operator-typed password is stashed under
    # ``evs-<env>_vcfInstaller`` on the first run; subsequent
    # runs read it back from there and skip the prompt entirely.
    ...
    # Step 3: drop the local installer VMFS now that vSAN is up.
    # Storage-vMotions any remaining VMs to vSAN before unmounting.
    # Phase 3 also handles the EBS-volume detach + delete in step 4
    # (destroy-ebs-volume), so this block is just the datastore unmount.

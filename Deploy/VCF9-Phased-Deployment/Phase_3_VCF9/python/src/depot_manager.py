...
class DepotManager:
    ...
    def sync_depot(
        ...
        while time.time() < deadline:
            ...
            logger.info("Depot sync status=%s", status)
            # Broadcom's installer uses a handful of synonyms for each
            # terminal state depending on version; accept them all.
...
def _summarize_bundle(b: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "downloadStatus": getattr(ds, "status", None),
        "downloadedSizeMB": getattr(ds, "downloaded_size", None),
        "message": getattr(ds, "message", None),
    }

...
class InstallerClient:
    ...
    Args:
        ...
        verify_tls: If False, disable TLS cert verification (typical for the
            installer's self-signed cert).
    ...
    def __init__(
        ...
        # Retry transient failures on the wire — the VCF installer has a
        # habit of resetting idle TCP connections on a surprisingly short
        # interval. Without this, a poll loop running for more than a few
        # minutes can crash with ConnectionResetError on the very next call
        # after an idle period, which is exactly where long downloads hurt.
        retry = Retry(
            total=5,
            connect=5,
            read=5,
    ...
    def _make_stub_config(self) -> StubConfiguration:
        ...
        Called both anonymously (for ``Tokens.create_token``) and after
        authentication — the session carries the ``Authorization`` header
        either way, so the same factory is enough.

        ``msg_protocol='rest'`` is critical: every installer stub is
        generated with ``is_vapi_rest=False`` (Swagger-REST), and the
        default ``'json'`` connector speaks JSON-RPC which the installer
        doesn't accept. Leaving this at the default surfaces as
    ...
    def refresh_token(self) -> bool:
        ...
        try:
            ...
            logger.info("Token refresh failed (%s); installer may be gone", e)
            return False
    ...
    def _ensure_authenticated(self) -> None:
        ...
        # Optional request/response debug. Set VCF_INSTALLER_DEBUG=1 to see
        # the exact wire JSON hitting /v1/sddcs and the exact error body the
        # installer returned. Useful for reverse-engineering undocumented
        # field requirements.
        import os
    ...
    def raw_get(self, path: str, **params: Any) -> Any:
        ...
        Used for endpoints the SDK doesn't expose (e.g. /v1/product-binaries
        — the 9.0+ replacement for /v1/bundles, not modeled by the 9.1 SDK).
        Reuses the SDK-authenticated session so Bearer auth + TLS-skip are
    ...
    def _attach_wire_debug(self) -> None:
        ...
        Triggered by the VCF_INSTALLER_DEBUG env var. Logs each POST/PATCH
        body going out and every non-2xx response body coming back.
